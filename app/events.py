...
async def start_face_recognition_service(database: Database, bot: SimpleLongPollBot) -> None:
    queue = Queue()  # создание очереди для обмена изображениями между процессами
    ...
    Process(target=watcher.run, args=(queue, CAMERA_ID)).start()  # запуск синхронную проверку камеры в другом процессе, чтобы не блокировать event loop
